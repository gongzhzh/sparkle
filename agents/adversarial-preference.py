'''
To record the user's preference profile, saving and restoring the thread state after every message.
This ensures that the agent remembers user preferences across sessions.

Input: User's input messages.
Output: Preference profile JSON.
'''
...
MODEL_ID_OPENROUTER = os.getenv("MODEL_ID_OPENROUTER")
API_KEY_OPENROUTER = os.getenv("API_KEY_OPENROUTER")
BASE_URL_OPENROUTER = os.getenv("BASE_URL_OPENROUTER")

async def main():
    """Interactive demo to record the user's preference profile."""
    
    print("\n" + "="*70)
    print("🧵 Apreference profile: Profile Save/Restore After Every Message")
    print("="*70)
    
    agent = OpenAIChatClient(
        model_id=f"{MODEL_ID_OPENROUTER}",
        api_key=f"{API_KEY_OPENROUTER}",
        base_url=f"{BASE_URL_OPENROUTER}"
    ).create_agent(
        instructions=
        """You are a preference settup assistant.
        You need to help the user set up their preferences during the exploratory testing session.
        Remember to ask questions to understand the user's preferences. 
        Remember everything the user tells you and refer back to it.
        """,
        name="adversarial-preference-agent"
    )
    ...
    # ).create_agent(
    #     instructions="""You are a preference settup assistant.
    #     You need to help the user set up their preferences during the exploratory testing session.
    #     Remember to ask questions to understand the user's preferences. 
    #     Remember everything the user tells you and refer back to it.""",
    #     name="adversarial-preference-agent"
    ...
    # Try to load existing pregerence from file, or create new one
    print("📋 Checking for existing preference file...")
    thread = None
    message_count = 0
